...
class DisplayMenu(BaseManager):
    """
    Display settings menu rendered via the central MenuManager list view.
    Submenus:
    ...
    def __init__(
        ...
        # Static options (labels only; we’ll convert to items when showing)
        ...
        self.logger.info(
            "DisplayMenu initialised. menu_controller=%s",
    ...
    def _items_for_menu(self, menu_name: str) -> List[Dict]:
        ...
        if menu_name == "modern_vu_styles":
            rows = [{"title": s, "type": "action", "action": "set_modern_vu_style", "value": s} for s in self.modern_vu_styles]
            rows.append({"title": "Back", "type": "back"})
            return rows

        if menu_name == "simple":
            rows = [{"title": s, "type": "action", "action": "set_display_mode", "value": s} for s in self.simple_modes]
            rows.append({"title": "Back", "type": "back"})
            return rows

        if menu_name == "brightness":
            rows = [{"title": b, "type": "action", "action": "set_brightness", "value": b} for b in self.brightness_items]
            rows.append({"title": "Back", "type": "back"})
            return rows
    ...
    def _show_current_menu(self):
        if not self.menu_controller:
            ...
            return
        self.logger.info("DisplayMenu: show_list -> %s", self._title_for_menu(self.current_menu))
    ...
    def _toggle_spectrum(self):
        ...
        self.logger.info("DisplayMenu: Spectrum => %s", "On" if self.spectrum_enabled else "Off")
    ...
    def _handle_display_mode(self, mode_name: str):
        ...
        if val:
            self.logger.info("DisplayMenu: Setting display mode => %s", val)
            self.mode_manager.set_display_mode(val)
        else:
            self.logger.warning("DisplayMenu: Unknown display mode => %s", mode_name)
    ...
    def _handle_modern_vu_style(self, style_name: str):
        ...
        if selected:
            self.logger.info("DisplayMenu: Setting Modern-VU spectrum mode => %s", selected)
            self.mode_manager.config["modern_spectrum_mode"] = selected
            self.mode_manager.set_display_mode("modern")
            self.mode_manager.save_preferences()
        else:
            self.logger.warning("DisplayMenu: Unknown Modern-VU style => %s", style_name)
    ...
    def _handle_brightness(self, level: str):
        ...
        if val is not None and hasattr(self.display_manager.oled, "contrast"):
            try:
                ...
                self.logger.info("DisplayMenu: Brightness => %s (%d)", level, val)
            except Exception as e:
                self.logger.error("DisplayMenu: Failed to set brightness => %s", e)
            self.mode_manager.config["oled_brightness"] = val
            self.mode_manager.save_preferences()
        else:
            self.logger.warning("DisplayMenu: Unknown brightness => %s", level)
    ...
    def _handle_screensaver(self, sel: str):
        ...
        self.logger.info("DisplayMenu: screensaver_type => %s", chosen)
        self.mode_manager.save_preferences()

    def _handle_screensaver_timer_value(self, seconds: int):
        self.mode_manager.config["screensaver_timeout"] = seconds
        self.logger.info("DisplayMenu: screensaver_timeout => %ds", seconds)
        self.mode_manager.save_preferences()
